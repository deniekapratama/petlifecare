...
class Sales extends CI_Controller {
	...
	{
		$data['title'] = 'Admin | Order Accepted';
		$data['sales_data'] = $this->msales->sales();
	...
	{
		$data['title'] = 'Admin | Order Cancelled';
		$data['sales_fail_data'] = $this->msales->sales_fail();
		$data['confirm_data_index']		= count($this->msales->data_confirm());
		$data['confirm_data_care_index']		= count($this->mcare->data_confirm());
		$data['confirm_data_daycare_index']		= count($this->mdaycare->data_confirm());
		$total_notif = $data['confirm_data_index'] + $data['confirm_data_daycare_index'] + $data['confirm_data_care_index'];
		...
		$this->load->view('templates_admin/sidebar');
		$this->load->view('templates_admin/navbar', $data);
		$this->load->view('admin/sales/sales_fail', $data);
		$this->load->view('templates_admin/footer');
	...
	public function search(){
		...
		$dat1 = $this->msales->filter("pesanan_barang",$input["start_date"], $input["end_date"]);
		foreach ($dat1 as $rows){
            ...
            'alamat' => $rows->alamat,
            'tgl_pemesanan' => exDate($rows->tgl_pemesanan),
	...
	public function confirm_order()
	{
		...
		$data['confirm_data'] = $this->msales->data_confirm();
		$data['confirm_data_index']		= count($this->msales->data_confirm());
		$data['confirm_data_care_index']		= count($this->mcare->data_confirm());
		$data['confirm_data_daycare_index']		= count($this->mdaycare->data_confirm());
		$total_notif = $data['confirm_data_index'] + $data['confirm_data_daycare_index'] + $data['confirm_data_care_index'];
		...
		$this->load->view('templates_admin/sidebar');
		$this->load->view('templates_admin/navbar', $data);
		$this->load->view('admin/sales/confirm_sales', $data);
		$this->load->view('templates_admin/footer');
	...
	{
		$this->msales->confirm_update($id);
		redirect('confirm_order');
	}

	public function tolak($id)
	{
		$this->msales->tolak_update($id);
		redirect('confirm_order');
	}
}
